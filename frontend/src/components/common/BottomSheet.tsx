    ...
    return (
        ...
        <div className={`fixed inset-0 z-50 transition-all duration-300 ${isOpen ? 'visible' : 'invisible'} `}>
            ...
            <div className={`absolute bottom-0 left-0 right-0 px-4 pb-16 transform bg-white rounded-t-[40px] shadow-[-4px_0_16px_rgba(0,0,0,0.1)] transition-transform duration-300 ease-in-out ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
                {title &&
                    <div className="relative px-1 py-7 flex flex-row items-center justify-center">
                        <span className="title-n font-semibold text-grey-15">{title}</span>
                        <button
                            onClick={onClose}
                            className="absolute right-4 w-10 h-10 p-2 bg-gradient-to-b from-white/0 to-white rounded-3xl shadow-[0px_5px_20px_-5px_rgba(35,48,59,0.25)] outline-1 outline-offset-[-1px] outline-white/70 backdrop-blur-[50px] inline-flex justify-center items-center overflow-hidden transition-all duration-150 active:bg-black/20">
